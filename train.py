...
class FaultNet(nn.Module):
    def __init__(self, f_in=3, hidden=64):
        ...
        self.head = nn.Conv1d(hidden, 1, 1)  # -> (B,1,16)

    def forward(self, x):
        # x: (B,16,3) -> (B,3,16)
        x = x.transpose(1, 2)
        h = self.backbone(x)
        logits = self.head(h).squeeze(1)  # (B,16)
        return logits

def metrics_from_logits(logits, y_true, thr):
    """
    logits: (B,16)
    y_true: (B,16) float 0/1
    """
...
def main():
    ...
    # Если отказов мало, полезно взвесить positive class
    # pos_weight = (neg/pos) по всему train
    ...
    for epoch in range(1, 21):
        ...
        # val: подобрать лучший порог и оценить
        ...
        print(f"Epoch {epoch:02d} | train_loss={total/len(train_ds):.4f} | "
    ...
    print("\nTEST:")
    print(f"threshold={best_thr:.2f} | F1={f1:.3f} | Precision={p:.3f} | Recall={r:.3f} | ExactMatch={ex:.3f}")
    ...
    print("\nSaved: faultnet.pt, threshold.json")
